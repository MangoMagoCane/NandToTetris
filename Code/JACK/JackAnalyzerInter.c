...
_Bool 
# 21 "../utilities.c"
    checkExtension(char *filename_p, char **extension_pp, char *extension)
{
    char *p = filename_p;
    char *dot_p = 
# 24 "../utilities.c" 3 4
...
# 37 "JackTokenizer.c" 3 4
_Bool 
# 37 "JackTokenizer.c"
    isType(Token *token_p);

# 38 "JackTokenizer.c" 3 4
_Bool 
# 38 "JackTokenizer.c"
    isIdentifier(Token *token_p);
# 47 "JackTokenizer.c"
...
static FILE *g_curr_file_p = 
# 79 "JackTokenizer.c" 3 4
                            ((void *)0)
# 79 "JackTokenizer.c"
                                ;
static char *g_line_buf_p = 
# 80 "JackTokenizer.c" 3 4
                           ((void *)0)
# 80 "JackTokenizer.c"
                               ;

Token *g_pushback_buf[128];
static int g_pushback_i = 0;

Token *curr_token;
char g_curr_token[1024];


# 88 "JackTokenizer.c" 3 4
_Bool 
# 88 "JackTokenizer.c"
    g_tokens_left = 
# 88 "JackTokenizer.c" 3 4
...
void copyToken(Token **token_pp, Token *token_p)
{
    ...
    default:
        fprintf(
# 127 "JackTokenizer.c" 3 4
               stderr
# 127 "JackTokenizer.c"
                     , "ERR: Token type: %d", token_p->type);
        *token_pp = 
# 128 "JackTokenizer.c" 3 4
                   ((void *)0)
# 128 "JackTokenizer.c"
                       ;
        return;
...
void setTokenizerFile(FILE *fp)
{
    g_curr_file_p = fp;
    g_line_buf_p = 
# 139 "JackTokenizer.c" 3 4
                  ((void *)0)
# 139 "JackTokenizer.c"
                      ;
    g_empty_line = 
# 140 "JackTokenizer.c" 3 4
                  1
# 140 "JackTokenizer.c"
                      ;
    g_tokens_left = 
# 141 "JackTokenizer.c" 3 4
                   1
# 141 "JackTokenizer.c"
                       ;
    g_in_multi_line = 
# 142 "JackTokenizer.c" 3 4
                     0
# 142 "JackTokenizer.c"
                          ;
    g_curr_line = 0;
...
void pushback(Token *token_p) {
    if (g_pushback_i >= 128) {
        fprintf(
# 148 "JackTokenizer.c" 3 4
               stderr
# 148 "JackTokenizer.c"
                     , "ERR: Token pushback limit reached %d\n", 128);
        return;
    }

    Token **tok_pp = &g_pushback_buf[g_pushback_i];
    if (token_p == 
# 153 "JackTokenizer.c" 3 4
                  ((void *)0)
# 153 "JackTokenizer.c"
                      ) {
        token_p = curr_token;
...
load_line:
    while (g_empty_line || g_in_multi_line) {
        if (fgets(g_line_buf, sizeof (g_line_buf), g_curr_file_p) == 
# 180 "JackTokenizer.c" 3 4
                                                                    ((void *)0)
# 180 "JackTokenizer.c"
                                                                        ) {
            g_tokens_left = 
# 181 "JackTokenizer.c" 3 4
                           0
# 181 "JackTokenizer.c"
                                ;
            return curr_token;
...
# 223 "JackTokenizer.c"
    ...
    if (g_line_buf_p[0] == '/' && g_line_buf_p[1] == '*') {
        char* strstr_p = strstr(g_line_buf, "*/");
        if (strstr_p == 
# 231 "JackTokenizer.c" 3 4
                       ((void *)0)
# 231 "JackTokenizer.c"
                           ) {

            g_in_multi_line = 
# 233 "JackTokenizer.c" 3 4
                             1
# 233 "JackTokenizer.c"
                                 ;
            goto load_line;
    ...
    c = *g_line_buf_p;
    for (uint i = 0; i < (sizeof (g_symbols) / sizeof (g_symbols[0])); ++i) {
        if (c == g_symbols[i]) {
    ...
    if (int_const_len > 0) {
        ...
        curr_token->fixed_val.len = int_const_len + 1;
        g_line_buf_p += int_const_len;
        return curr_token;
    }

    uint i = 0;
    while ((c = *g_line_buf_p++) == '_' || 
# 282 "JackTokenizer.c" 3 4
                                          ((*__ctype_b_loc ())[(int) ((
# 282 "JackTokenizer.c"
                                          c
# 282 "JackTokenizer.c" 3 4
...
# 282 "JackTokenizer.c"
    ...
    if (i > 0) {
        g_curr_token[i] = '\0';
        g_line_buf_p--;
        for (Keyword i = 0; i < (sizeof (g_keywords) / sizeof (g_keywords[0])); ++i) {
...
# 307 "JackTokenizer.c"
    ...
    if (is_unary) {
        arr_p = g_unary_ops;
        len = (sizeof (g_unary_ops) / sizeof (g_unary_ops[0]));
    } else {
        arr_p = g_ops;
        len = (sizeof (g_ops) / sizeof (g_ops[0]));
    }
...
# 18 "SymbolTable.c" 3 4
_Bool 
# 18 "SymbolTable.c"
    addSymtabEntry(VariableSymtabEntry *symtab, char *name, char *type, char *kind);
VariableSymtabEntry *searchSymtab(char *name);

static VariableSymtabEntry g_global_symtab[128];
static VariableSymtabEntry g_subroutine_symtab[128];
...
VariableSymtabEntry *lookupSymtabEntry(char *name)
{
    for (uint i = 0; g_subroutine_symtab[i].name[0] && i < 128; ++i) {
        if (strcmp(name, g_subroutine_symtab[i].name) == 0) {
            return &g_subroutine_symtab[i];
        }
    }

    for (uint i = 0; g_global_symtab[i].name[0] && i < 128; ++i) {
...
# 75 "SymbolTable.c"
    ...
    strncpy(symtab[i].name, name, (sizeof (((VariableSymtabEntry *) 0)->name)));
    strncpy(symtab[i].type, type, (sizeof (((VariableSymtabEntry *) 0)->type)));
    strncpy(symtab[i].kind, kind, (sizeof (((VariableSymtabEntry *) 0)->kind)));

    return 
# 87 "SymbolTable.c" 3 4
          1
# 87 "SymbolTable.c"
              ;
}
...
VariableSymtabEntry *searchSymtab(char *name)
{
    ...
    return 
# 98 "SymbolTable.c" 3 4
          ((void *)0)
# 98 "SymbolTable.c"
              ;
}
# 12 "CompilationEngine.c" 2
...
# 28 "CompilationEngine.c" 3 4
_Bool 
# 28 "CompilationEngine.c"
    process(TokenType type, uint64_t data, ProcessOptional optional);
void compileClass();
...
static const char *convertOpToVM(char op, 
# 45 "CompilationEngine.c" 3 4
                                         _Bool 
# 45 "CompilationEngine.c"
                                              isUnary);
# 79 "CompilationEngine.c"
static FILE *g_writer_fp;
static char g_writer_name_buf[
# 80 "CompilationEngine.c" 3 4
                             255
# 80 "CompilationEngine.c"
                                     ];
static uint g_indent_amount = 0;
static 
# 82 "CompilationEngine.c" 3 4
      _Bool 
# 82 "CompilationEngine.c"
           g_print_xml = 
# 82 "CompilationEngine.c" 3 4
                         1
# 82 "CompilationEngine.c"
...
void setWriterOutputFile(FILE *fp, char *file_path)
{
    ...
    size_t filename_len = 
# 91 "CompilationEngine.c" 3 4
                         (((
# 91 "CompilationEngine.c"
                         extension_p - filename_p
# 91 "CompilationEngine.c" 3 4
                         )<(
# 91 "CompilationEngine.c"
                         sizeof (g_writer_name_buf)
# 91 "CompilationEngine.c" 3 4
                         ))?(
# 91 "CompilationEngine.c"
                         extension_p - filename_p
...
void printXML(const Token *token_p, const char *grammar_elem) {
    ...
    switch (token_p->type) {
    case KEYWORD:
        { fprintf(g_writer_fp, "%*s<%s> %" "s" " </%s>\n", g_indent_amount, " ", grammar_elem, g_keywords[token_p->fixed_val.keyword], grammar_elem); };
        break;
    case SYMBOL: ;
        char symbol = token_p->fixed_val.symbol;
        switch (symbol) {
        case '<':
            { fprintf(g_writer_fp, "%*s<%s> %" "s" " </%s>\n", g_indent_amount, " ", grammar_elem, "&lt;", grammar_elem); };
            break;
        case '>':
            { fprintf(g_writer_fp, "%*s<%s> %" "s" " </%s>\n", g_indent_amount, " ", grammar_elem, "&gt;", grammar_elem); };
            break;
        ...
        case '&':
            ...
            break;
        default:
            { fprintf(g_writer_fp, "%*s<%s> %" "c" " </%s>\n", g_indent_amount, " ", grammar_elem, symbol, grammar_elem); };
            break;
        }
        break;
    case IDENTIFIER:
    ...
    case STRING_CONST:
        { fprintf(g_writer_fp, "%*s<%s> %" "s" " </%s>\n", g_indent_amount, " ", grammar_elem, token_p->var_val, grammar_elem); };
        break;
    default:
        fprintf(
# 131 "CompilationEngine.c" 3 4
               stderr
# 131 "CompilationEngine.c"
                     , "ERR: Cannot print XML Token type: %d", curr_token->type);
        break;
    }
}


# 136 "CompilationEngine.c" 3 4
_Bool 
# 136 "CompilationEngine.c"
    process(TokenType type, uint64_t _data, ProcessOptional optional)
{
    
# 138 "CompilationEngine.c" 3 4
   _Bool 
# 138 "CompilationEngine.c"
        retval = 
# 138 "CompilationEngine.c" 3 4
                 0
# 138 "CompilationEngine.c"
    ...
    if (type == curr_token->type) {
        ...
        case STRING_CONST:
            ...
            retval = 
# 154 "CompilationEngine.c" 3 4
                    1
# 154 "CompilationEngine.c"
                        ;
            break;
        default:
            { fprintf(
# 157 "CompilationEngine.c" 3 4
           stderr
# 157 "CompilationEngine.c"
           , "%s\nERR: Invalid " "Token type" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
            break;
        }
    }
    ...
    if (retval) {
        ...
        { fprintf(
# 166 "CompilationEngine.c" 3 4
       stderr
# 166 "CompilationEngine.c"
       , "%s\nERR: Invalid " "Token" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
    }
...
void compileClass()
{
    { fprintf(g_writer_fp, "%*s<" "class" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    resetSymtab(g_global_symtab);

    process(KEYWORD, CLASS, MAND);
    process(IDENTIFIER, 
# 178 "CompilationEngine.c" 3 4
                       ((void *)0)
# 178 "CompilationEngine.c"
                           , MAND);
    process(SYMBOL, '{', MAND);
    ...
    printSymtabs(
# 198 "CompilationEngine.c" 3 4
                1
# 198 "CompilationEngine.c"
                    , 
# 198 "CompilationEngine.c" 3 4
                      0
# 198 "CompilationEngine.c"
                           );
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "class" ">\n", g_indent_amount, ""); };
}

void compileClassVarDec()
{
    { fprintf(g_writer_fp, "%*s<" "classVarDec" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    char type_buf[1024];
    char kind_buf[1024];

    strncpy(kind_buf, g_keywords[curr_token->fixed_val.keyword], sizeof (kind_buf));
    if (process(KEYWORD, STATIC, OPTNL) || process(KEYWORD, FIELD, MAND)) {}
    if (!isType(curr_token)) {
        { fprintf(
# 211 "CompilationEngine.c" 3 4
       stderr
# 211 "CompilationEngine.c"
       , "%s\nERR: Invalid " "type" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
    }
    if (curr_token->type == IDENTIFIER) {
        strncpy(type_buf, curr_token->var_val, sizeof (type_buf));
    } else {
        strncpy(type_buf, g_keywords[curr_token->fixed_val.keyword], sizeof (type_buf));
    }
    printXML(curr_token, g_token_types[curr_token->type]);
    advance();


    { if (!addSymtabEntry(g_global_symtab, curr_token->var_val, type_buf, kind_buf)) { { fprintf(
# 222 "CompilationEngine.c" 3 4
   stderr
# 222 "CompilationEngine.c"
   , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
    process(IDENTIFIER, 
# 223 "CompilationEngine.c" 3 4
                       ((void *)0)
# 223 "CompilationEngine.c"
                           , MAND);

    while (process(SYMBOL, ',', OPTNL)) {

        { if (!addSymtabEntry(g_global_symtab, curr_token->var_val, type_buf, kind_buf)) { { fprintf(
# 227 "CompilationEngine.c" 3 4
       stderr
# 227 "CompilationEngine.c"
       , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
        process(IDENTIFIER, 
# 228 "CompilationEngine.c" 3 4
                           ((void *)0)
# 228 "CompilationEngine.c"
                               , MAND);
    }
    process(SYMBOL, ';', MAND);

    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "classVarDec" ">\n", g_indent_amount, ""); };
}

void compileSubroutine()
{
    { fprintf(g_writer_fp, "%*s<" "subroutineDec" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    if (process(KEYWORD, CONSTRUCTOR, OPTNL)) {
        ...
        { if (!addSymtabEntry(g_subroutine_symtab, "this", g_writer_name_buf, "arg")) { { fprintf(
# 243 "CompilationEngine.c" 3 4
       stderr
# 243 "CompilationEngine.c"
       , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
    }
    ...
    process(IDENTIFIER, 
# 254 "CompilationEngine.c" 3 4
                       ((void *)0)
# 254 "CompilationEngine.c"
                           , MAND);
    process(SYMBOL, '(', MAND);
    ...
    printSymtabs(
# 260 "CompilationEngine.c" 3 4
                0
# 260 "CompilationEngine.c"
                     , 
# 260 "CompilationEngine.c" 3 4
                       1
# 260 "CompilationEngine.c"
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "subroutineDec" ">\n", g_indent_amount, ""); };
}

void compileParameterList()
{
    { fprintf(g_writer_fp, "%*s<" "parameterList" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    char type_buf[1024];
    ...
    if (isType(curr_token)) {
        ...
        advance();
        { if (!addSymtabEntry(g_subroutine_symtab, curr_token->var_val, type_buf, "arg")) { { fprintf(
# 279 "CompilationEngine.c" 3 4
       stderr
# 279 "CompilationEngine.c"
       , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
        process(IDENTIFIER, 
# 280 "CompilationEngine.c" 3 4
                           ((void *)0)
# 280 "CompilationEngine.c"
                               , MAND);
        while (process(SYMBOL, ',', OPTNL)) {
            if (!isType(curr_token)) {
                { fprintf(
# 283 "CompilationEngine.c" 3 4
               stderr
# 283 "CompilationEngine.c"
               , "%s\nERR: Invalid " "type" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
            }
            printXML(curr_token, g_token_types[curr_token->type]);
            if (curr_token->type == IDENTIFIER) {
            ...
            advance();
            { if (!addSymtabEntry(g_subroutine_symtab, curr_token->var_val, type_buf, "arg")) { { fprintf(
# 293 "CompilationEngine.c" 3 4
           stderr
# 293 "CompilationEngine.c"
           , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
            process(IDENTIFIER, 
# 294 "CompilationEngine.c" 3 4
                               ((void *)0)
# 294 "CompilationEngine.c"
                                   , MAND);
        }
    }

    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "parameterList" ">\n", g_indent_amount, ""); };
}

void compileSubroutineBody()
{
    { fprintf(g_writer_fp, "%*s<" "subroutineBody" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "subroutineBody" ">\n", g_indent_amount, ""); };
}

void compileVarDec()
{
    { fprintf(g_writer_fp, "%*s<" "varDec" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    char type_buf[1024];

    process(KEYWORD, VAR, MAND);
    if (!isType(curr_token)) {
        { fprintf(
# 322 "CompilationEngine.c" 3 4
       stderr
# 322 "CompilationEngine.c"
       , "%s\nERR: Invalid " "type" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
    }
    if (curr_token->type == IDENTIFIER) {
        strncpy(type_buf, curr_token->var_val, sizeof (type_buf));
    ...
    { if (!addSymtabEntry(g_subroutine_symtab, curr_token->var_val, type_buf, "var")) { { fprintf(
# 332 "CompilationEngine.c" 3 4
   stderr
# 332 "CompilationEngine.c"
   , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
    process(IDENTIFIER, 
# 333 "CompilationEngine.c" 3 4
                       ((void *)0)
# 333 "CompilationEngine.c"
                           , MAND);

    while (process(SYMBOL, ',', OPTNL)) {
        { if (!addSymtabEntry(g_subroutine_symtab, curr_token->var_val, type_buf, "var")) { { fprintf(
# 336 "CompilationEngine.c" 3 4
       stderr
# 336 "CompilationEngine.c"
       , "%s\nERR: Invalid " "reused variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); }; } };
        process(IDENTIFIER, 
# 337 "CompilationEngine.c" 3 4
                           ((void *)0)
# 337 "CompilationEngine.c"
                               , MAND);
    }
    process(SYMBOL, ';', MAND);

    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "varDec" ">\n", g_indent_amount, ""); };
}

void compileStatements()
{
    { fprintf(g_writer_fp, "%*s<" "statements" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "statements" ">\n", g_indent_amount, ""); };
}

void compileLet()
{
    { fprintf(g_writer_fp, "%*s<" "letStatement" ">\n", g_indent_amount, ""); g_indent_amount += 2; };

    process(KEYWORD, LET, MAND);
    if (curr_token->type != IDENTIFIER) {
        { fprintf(
# 378 "CompilationEngine.c" 3 4
       stderr
# 378 "CompilationEngine.c"
       , "%s\nERR: Invalid " "variable name" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
    }
    printXML(curr_token, g_token_types[curr_token->type]);
    advance();
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "letStatement" ">\n", g_indent_amount, ""); };
}

void compileIf()
{
    { fprintf(g_writer_fp, "%*s<" "ifStatement" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "ifStatement" ">\n", g_indent_amount, ""); };
}

void compileWhile()
{
    { fprintf(g_writer_fp, "%*s<" "whileStatement" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "whileStatement" ">\n", g_indent_amount, ""); };
}

void compileDo()
{
    { fprintf(g_writer_fp, "%*s<" "doStatement" ">\n", g_indent_amount, ""); g_indent_amount += 2; };

    process(KEYWORD, DO, MAND);
    Token *identifier_p;
    copyToken(&identifier_p, curr_token);
    process(IDENTIFIER, 
# 435 "CompilationEngine.c" 3 4
                       ((void *)0)
# 435 "CompilationEngine.c"
                           , MAND);
    if (process(SYMBOL, '(', OPTNL)) {
        ...
        process(IDENTIFIER, 
# 440 "CompilationEngine.c" 3 4
                           ((void *)0)
# 440 "CompilationEngine.c"
                               , MAND);
        process(SYMBOL, '(', MAND);
        compileExpressionList();
        process(SYMBOL, ')', MAND);
        ...
        { fprintf(
# 445 "CompilationEngine.c" 3 4
       stderr
# 445 "CompilationEngine.c"
       , "%s\nERR: Invalid " "symbol" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
    }
    process(SYMBOL, ';', MAND);
    free(identifier_p);

    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "doStatement" ">\n", g_indent_amount, ""); };
}

void compileReturn()
{
    { fprintf(g_writer_fp, "%*s<" "returnStatement" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "returnStatement" ">\n", g_indent_amount, ""); };
}

void compileExpression()
{
    { fprintf(g_writer_fp, "%*s<" "expression" ">\n", g_indent_amount, ""); g_indent_amount += 2; };

    compileTerm();
    while (isOp(curr_token, 
# 471 "CompilationEngine.c" 3 4
                           0
# 471 "CompilationEngine.c"
                                )) {
        const char *vm_op_p = convertOpToVM(curr_token->fixed_val.symbol, 
# 472 "CompilationEngine.c" 3 4
                                                                         0
# 472 "CompilationEngine.c"
                                                                              );
        printXML(curr_token, g_token_types[curr_token->type]);
        advance();
        compileTerm();
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "expression" ">\n", g_indent_amount, ""); };
}

void compileTerm()
{
    { fprintf(g_writer_fp, "%*s<" "term" ">\n", g_indent_amount, ""); g_indent_amount += 2; };

    const TokenType type = curr_token->type;
    const TokenType keyword = curr_token->fixed_val.keyword;
    
# 488 "CompilationEngine.c" 3 4
   _Bool 
# 488 "CompilationEngine.c"
        single_token_term = 
# 488 "CompilationEngine.c" 3 4
                            1
# 488 "CompilationEngine.c"
    ...
    if (type == INT_CONST) {
        ...
        single_token_term = 
# 500 "CompilationEngine.c" 3 4
                           0
# 500 "CompilationEngine.c"
                                ;
    }
    ...
    if (single_token_term) {
        printXML(curr_token, g_token_types[type]);
        advance();
        { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "term" ">\n", g_indent_amount, ""); };
        return;
    }
...
# 514 "CompilationEngine.c" 3 4
                           ((void *)0)
# 514 "CompilationEngine.c"
                               , OPTNL)) {
        if (process(SYMBOL, '[', OPTNL)) {
            compileExpression();
            process(SYMBOL, ']', MAND);
            ...
            process(IDENTIFIER, 
# 523 "CompilationEngine.c" 3 4
                               ((void *)0)
# 523 "CompilationEngine.c"
                                   , MAND);
            process(SYMBOL, '(', MAND);
            expression_count = compileExpressionList();
            process(SYMBOL, ')', MAND);
        ...
        compileExpression();
        process(SYMBOL, ')', MAND);
    } else if (isOp(curr_token, 
# 533 "CompilationEngine.c" 3 4
                               1
# 533 "CompilationEngine.c"
                                   )) {
        const char *vm_op_p = convertOpToVM(curr_token->fixed_val.symbol, 
# 534 "CompilationEngine.c" 3 4
                                                                         1
# 534 "CompilationEngine.c"
                                                                             );
        printXML(curr_token, g_token_types[curr_token->type]);
        advance();
        compileTerm();
        ...
        { fprintf(
# 540 "CompilationEngine.c" 3 4
       stderr
# 540 "CompilationEngine.c"
       , "%s\nERR: Invalid " "term" " on line: %d\n", g_writer_name_buf, g_curr_line); printToken(curr_token); exit(1); };
    }
...
uint compileExpressionList() {
    ...
    { fprintf(g_writer_fp, "%*s<" "expressionList" ">\n", g_indent_amount, ""); g_indent_amount += 2; };
    ...
    { g_indent_amount -= 2; fprintf(g_writer_fp, "%*s</" "expressionList" ">\n", g_indent_amount, ""); };
    return expression_count;
}

static const char *convertOpToVM(char op, 
# 564 "CompilationEngine.c" 3 4
                                         _Bool 
# 564 "CompilationEngine.c"
                                              isUnary) {
    ...
    for (uint i = 0; i < (sizeof (g_ops) / sizeof (g_ops[0])); ++i) {
        if (op == g_ops[i]) {
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        fprintf(
# 15 "JackAnalyzer.c" 3 4
               stderr
# 15 "JackAnalyzer.c"
                     , "usage: ./JackAnalyzer (file.jack, dir)\n");
        retval = INVALID_ARG_CNT;
        goto exit;
    }

    
# 20 "JackAnalyzer.c" 3 4
   _Bool 
# 20 "JackAnalyzer.c"
        parse_dir = 
# 20 "JackAnalyzer.c" 3 4
...
# 20 "JackAnalyzer.c"
    ...
    if (checkExtension(path_p, &extension_p, "jack") == 
# 23 "JackAnalyzer.c" 3 4
                                                       0
# 23 "JackAnalyzer.c"
                                                            ) {
        if (extension_p[0] == '.') {
            fprintf(
# 25 "JackAnalyzer.c" 3 4
                   stderr
# 25 "JackAnalyzer.c"
                         , "file path: %s has invalid extension: %s\n", path_p, extension_p);
            retval = INVALID_FILE_EXTNSN;
            goto exit;
        }
        parse_dir = 
# 29 "JackAnalyzer.c" 3 4
                   1
# 29 "JackAnalyzer.c"
                       ;
    }
    ...
    if (parse_dir) {
        chdir(argv[1]);
        if ((dir_input = opendir(".")) == 
# 36 "JackAnalyzer.c" 3 4
                                         ((void *)0)
# 36 "JackAnalyzer.c"
                                             ) {
            fprintf(
# 37 "JackAnalyzer.c" 3 4
                   stderr
# 37 "JackAnalyzer.c"
                         , "cannot open: %s\n", argv[1]);
            retval = INVALID_DIR;
...
# 42 "JackAnalyzer.c" 3 4
                                              ((void *)0)
# 42 "JackAnalyzer.c"
                                                  ) {
            fprintf(
# 43 "JackAnalyzer.c" 3 4
                   stderr
# 43 "JackAnalyzer.c"
                         , "cannot open: %s\n", argv[1]);
            retval = INVALID_FILE;
    ...
    curr_token = malloc(sizeof (*curr_token) + (sizeof (curr_token->var_val[1024])));

    if (parse_dir) {
        struct dirent *dirent_p;
        while ((dirent_p = readdir(dir_input)) != 
# 54 "JackAnalyzer.c" 3 4
                                                 ((void *)0)
# 54 "JackAnalyzer.c"
                                                     ) {
            char d_name_buf[
# 55 "JackAnalyzer.c" 3 4
                           255
# 55 "JackAnalyzer.c"
                                   ];
            char output_name[
# 56 "JackAnalyzer.c" 3 4
                            4096
# 56 "JackAnalyzer.c"
                                    ];
            ...
            if ((f_input = fopen(d_name_buf, "r")) == 
# 61 "JackAnalyzer.c" 3 4
                                                     ((void *)0)
# 61 "JackAnalyzer.c"
                                                         ) {
                fprintf(
# 62 "JackAnalyzer.c" 3 4
                       stderr
# 62 "JackAnalyzer.c"
                             , "cannot open: %s\n", d_name_buf);
                retval = INVALID_FILE;
            ...
            if (f_output == 
# 69 "JackAnalyzer.c" 3 4
                           ((void *)0)
# 69 "JackAnalyzer.c"
                               ) {
                fprintf(
# 70 "JackAnalyzer.c" 3 4
                       stderr
# 70 "JackAnalyzer.c"
...
# 81 "JackAnalyzer.c"
        ...
        FILE *f_output = fopen(output_name, "w");
        if (f_output == 
# 84 "JackAnalyzer.c" 3 4
                       ((void *)0)
# 84 "JackAnalyzer.c"
                           ) {
            fprintf(
# 85 "JackAnalyzer.c" 3 4
                   stderr
# 85 "JackAnalyzer.c"
                         , "cannot open output file: %s\n", argv[1]);
            retval = INVALID_FILE;
